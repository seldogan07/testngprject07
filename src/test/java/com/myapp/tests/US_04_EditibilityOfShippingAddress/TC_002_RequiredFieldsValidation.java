...
public class TC_002_RequiredFieldsValidation {
    ...
    String zipCode;
    @Test(priority = 2)
    public void TC_002_Test() throws Exception {
        homePage = new PearlyMarketHomePage();
        ...
        //4. Leave one or more required fields (First Name, Last Name, Country/Region, Street Address, Town/City, State, ZIP Code) empty.
        passAndCaptureScreenshot("Enters credentials except First Name ... ");
        ...
        //6. Verify that appropriate error message ("First name is a required field.") is displayed for the empty field.
        passAndCaptureScreenshot("Verifies the error message 'First name is a required field.' is displayed for the empty field... ");
        String errorMessage = waitForVisibility(Driver.getDriver().findElement(By.xpath("//ul[@role='alert']")),5).getText();
        assertEquals("First name is a required field.",errorMessage);

        //7. Ensure that the Shipping Address is not added
        passAndCaptureScreenshot("Ensures that the Shipping Address is not added... ");
        assertTrue(shippingAddressesPage.shippingFirstName.getText().equals(""));
        passAndCaptureScreenshot("TC_002_Required Fields Validation... is done ");
        flush();
